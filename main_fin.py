...
class TodoList(QDialog, TodolistUI_Dialog):
    ...
    def deleteItemList(self):
        ...
        for item in delTodoItems:
            ...
            del_item = db.child(userID).child("Todo_List").child("TO DO").child(item_list[0]).get()
            # print(del_item.val())
            for idx, dict in del_item.val().items():
                del_idx = idx
                for key, val in dict.items():
                    if (key == 'details' and val == item_list[-1]):
                        db.child(userID).child("Todo_List").child("TO DO").child(item_list[0]).child(del_idx).remove()
        ...
        for item in delProgItems:
            ...
            del_item = db.child(userID).child("Todo_List").child("IN PROGRESS").child(item_list[0]).get()
            # print(del_item.val())
            for idx, dict in del_item.val().items():
                del_idx = idx
                for key, val in dict.items():
                    if (key == 'details' and val == item_list[-1]):
                        db.child(userID).child("Todo_List").child("IN PROGRESS").child(item_list[0]).child(del_idx).remove()
            ...
            for idx, dict in del_item.val().items():
                ...
                for key, val in dict.items():
                    if (key == 'details' and val == item_list[-1]):
                        ...
                        db.child(userID).child("Todo_List").child("IN PROGRESS").child(item_list[0]).push(data)
            ...
            for idx, dict in del_item.val().items():
                ...
                for key, val in dict.items():
                    if (key == 'details' and val == item_list[-1]):
                        ...
                        db.child(userID).child("Todo_List").child("TO DO").child(item_list[0]).push(data)
                ...
                for idx, dict in item.val().items():
                    ...
                    for key, val in dict.items():
                        if (key == 'importance'):
                            ...
                            detail_item = (val)
                    list_item.setText(data.key() + " / " + import_item + " / " + detail_item)
